...
function Allnews() {
  ...
  const Deletehandler = async (id) => {
    try {
      await axios.delete('/api/news/deleteuploadednews/' + id,{timeout:1000})
                    ...
                    <tbody>
                      {displayedItems.map((user, index) => (
                        <tr >
                          ...
                          <td>{user.date}</td>
                          <td>{user.title}</td>
                          <td> <img src={`/news/images/` + user.image} style={{ width: '50px', height: '50px' }} /> </td>
...

                          {
                            type && type == "user" ? ('') : (
                              <td>
                                <Link to={`/Updates/Updatenews/${user._id}`} className='btn btn-primary'>Edit Record</Link>
