...
const filteredData = users.filter((user) =>
  user.country.toLowerCase().includes(searchInput.toLowerCase()) || user.city.toLowerCase().includes(searchInput.toLowerCase())
);
...
function getcalendar(){
  ...
  const Deletehandler = async (id) => {
    try {
      await axios.delete('/api/calender/deletecalender/'+id,{ timeout: 1000 })
                        ...
                        <td>{user.check == "true" ? 'check' : 'uncheck'}</td>
                          {
                            type && type == "user" ? (''):( 
                              <td>
                              <Link to={`/Updates/UpdateCalender/${user._id}`} className='btn btn-primary'>Edit Record</Link>
