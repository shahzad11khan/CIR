...
// 
function getpartner(){
  axios.get('/api/partners/get/partners')
      .then((response) => {
...
// 
  ...
  const Deletehandler = async (id) => {
    // console.log(id)
    try {
      await axios.delete('/api/partners/delete/partners/' + id,{timeout:1000})
                    ...
                    <tbody>
                      {displayedItems.map((user, index) => (
                        <tr >
                          ...
                          <td><img src={`/partners/images/` + user.image} style={{ width: '50px', height: '50px' }} alt={user.image} /></td>
                          <td>{user.image}</td>
